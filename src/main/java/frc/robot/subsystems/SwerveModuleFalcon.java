...
public class SwerveModuleFalcon {
  ...
  private final TalonFX m_driveMotorMaster;
  private final TalonFX m_driverMotorSlave;
   ...
   *
   * @param driveMotorMasterChannel   ID for the drive motor.
   * @param turningMotorChannel ID for the turning motor.
   */
  public SwerveModuleFalcon(int driveMotorMasterChannel, int driveMotorSlaveChannel, int turningMotorChannel, int canCoderID, double absoluteTurnZeroDeg, boolean isRight) {

    this.absoluteTurnZeroDeg = absoluteTurnZeroDeg;

    m_driveMotorMaster = new TalonFX(driveMotorMasterChannel);
    m_driverMotorSlave = new TalonFX(driveMotorSlaveChannel);
    ...
    m_driveMotorMaster.setInverted(isRight ? TalonFXInvertType.CounterClockwise : TalonFXInvertType.Clockwise);
    m_driverMotorSlave.setInverted(isRight ? TalonFXInvertType.CounterClockwise : TalonFXInvertType.Clockwise);
    m_turnMotor.setInverted(TalonFXInvertType.CounterClockwise);

    m_driveMotorMaster.setNeutralMode(NeutralMode.Brake);
    m_driverMotorSlave.setNeutralMode(NeutralMode.Brake);
    m_turnMotor.setNeutralMode(NeutralMode.Brake);

    m_driverMotorSlave.follow(m_driveMotorMaster);

    m_driveMotorMaster.enableVoltageCompensation(true);
    m_driveMotorMaster.configVoltageCompSaturation(12.0);
    m_driveMotorMaster.configPeakOutputForward(+1.0f);
    m_driveMotorMaster.configPeakOutputReverse(-1.0f);
    ...
    m_driveMotorMaster.configSupplyCurrentLimit(supplyCurrentConfigs);
    m_driverMotorSlave.configSupplyCurrentLimit(supplyCurrentConfigs);
    ...
    m_driveMotorMaster.configStatorCurrentLimit(statorCurrentConfigs);
    m_driverMotorSlave.configStatorCurrentLimit(statorCurrentConfigs);
    m_turnMotor.configStatorCurrentLimit(statorCurrentConfigs);

    m_driveMotorMaster.config_kF(kWheelVelocitySlot, 0.04);
    m_driveMotorMaster.config_kP(kWheelVelocitySlot, 0.03);
    m_driveMotorMaster.config_kI(kWheelVelocitySlot, 0.00005);
    m_driveMotorMaster.config_kD(kWheelVelocitySlot, 1.5);
  
    m_driveMotorMaster.selectProfileSlot(kWheelVelocitySlot, 0);

    m_driverMotorSlave.config_kF(kWheelVelocitySlot, 0.04);
    m_driverMotorSlave.config_kP(kWheelVelocitySlot, 0.03);
    m_driverMotorSlave.config_kI(kWheelVelocitySlot, 0.00005);
    m_driverMotorSlave.config_kD(kWheelVelocitySlot, 1.5);
...
//            SwerveModuleState.optimize(desiredState, Rotation2d.fromDegrees(getTurnWheelAngleDegrees()));
    ...
    // Calculate the turning motor output from the turning PID controller.
        m_driveMotorMaster.set(TalonFXControlMode.Velocity, driveMetersPerSecondToTicksPer100ms(targetSpeed));
//         m_driverMotorSlave.set(TalonFXControlMode.Velocity, driveMetersPerSecondToTicksPer100ms(targetSpeed));
  ...
  public void setControlOff() {
    // Calculate the turning motor output from the turning PID controller.
    m_driveMotorMaster.set(TalonFXControlMode.PercentOutput,0);
  ...
  public void setWheelSpeed(double metersPerSec) {
    m_driveMotorMaster.set(TalonFXControlMode.Velocity, driveMetersPerSecondToTicksPer100ms(metersPerSec));
  }
  public void setWheelSpeedPercentBus(double percent) {
    m_driveMotorMaster.set(TalonFXControlMode.PercentOutput, percent);
  }

  // Zeros all the SwerveModule encoders.
  public void resetEncoders() {
    m_driveMotorMaster.setSelectedSensorPosition(0);
...
//    canEncoder.setPosition(0);
  ...
  public double getDriveVelocityNativeUnits() {
    return m_driveMotorMaster.getSelectedSensorVelocity(0);
  }
  ...
  public double getDrivePositionNativeUnits() {
    return m_driveMotorMaster.getSelectedSensorPosition(0);
  }
